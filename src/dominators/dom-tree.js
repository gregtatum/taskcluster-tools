...
function make_dom(opts) {
  if (array(opts)) opts = { nodes: opts };

  if (!array(opts.nodes))
    throw new Error(
      'You have to pass at least an array of nodes to make a Dominator walker object',
    );
  ...
  function alternative_idf(defs) {
    if (!frontiers)
      frontiers = frontiers_from_preds(simpleRefToSelf(nodes), idoms);
    ...
    while (worklist.length) {
      ...
      for (let i = 0, lng = front.length; i < lng; ++i) {
        const fi = front[i];
        if (_v[fi] !== true) worklist.push(fi);
      }
    }
  ...
  function _iterated_dominance_frontier(fn, defs) {
    ...
    while ((z = get_deepest_node()) !== null) visit((currentRoot = z));
  ...
  function strictlyDominates(from, to) {
    if (typeof to === 'number')
      return pre[to] > pre[from] && post[to] < post[from];
  ...
  function dominates(from, to) {
    if (typeof to === 'number')
      return from === to || (pre[to] > pre[from] && post[to] < post[from]);
  ...
  function _forDominanceFrontier(fn, from) {
    forDominates(
      (block) =>
        nodes[block].forEach((to) => !strictlyDominates(from, to) && fn(to)),
      from,
    );
  }
  ...
  function _forIteratedDominanceFrontier(fn, from) {
    ...
    while (worklist.length)
      _forDominanceFrontier(
        (otherBlock) => fn(otherBlock) && worklist.push(otherBlock),
        worklist.pop(),
      );
  }
