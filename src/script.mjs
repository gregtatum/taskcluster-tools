...
function setupProfilerButton(taskGroups, taskClusterURL) {
  elements.profiler.addEventListener("click", () => {
    // const profilerURL = "https://profiler.firefox.com/from-post-message/"
    const profilerURL = "http://localhost:4242/from-post-message/"

    const newWindow = window.open(profilerURL, "_blank");

    // Check if the new window has been opened successfully
    if (newWindow) {
      const message = {
        name: "inject-profile",
        profile: getProfile(taskGroups, taskClusterURL)
      }

      // Post the message to the new window
      setTimeout(() => {
        newWindow.postMessage(message, profilerURL);
      }, 1000)
    } else {
