...
export function getProfile(taskGroups, url) {
  ...
  for (let i = 0; i < taskGroups.length; i++) {
    ...
    thread.registerTime = timing.start - profileStartTime
    thread.unregisterTime = timing.end - profileStartTime
    console.log(`!!! sortedTasks`, sortedTasks);
    for (const {task} of sortedTasks) {
      if (!task.task.runs) {
        continue;
      }
      for (const run of task.task.runs) {
        ...
        if (end === null) {
          markers.endTime.push(startTime);
